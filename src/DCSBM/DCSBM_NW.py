...
class DCSBM:
    def __init__(self, graph, num_communities: int, seed: int, sub_graph: bool = False) -> None:
        self.graph = graph
        self.num_nodes = graph.number_of_nodes()
        self.num_communities = num_communities
        if sub_graph:
            self.A = self.calculate_adjancency_matrix_sub()
        else:
            self.A = self.calculate_adjacency_matrix()
        np.random.seed(seed)
        self.communities = np.random.randint(0, num_communities, self.num_nodes)
        # 追加
        self.community_matrtix = self.make_community_matrix()
    ...
    def calculate_adjancency_matrix_sub(self) -> np.ndarray:
        A_ = nx.to_numpy_array(self.graph)
        A = np.zeros((self.num_nodes, self.num_nodes))
    ...
    # 追加
    def make_community_matrix(self) -> np.ndarray:
        community_matrix = np.zeros((self.num_communities, self.num_nodes))
        for g in range(self.num_communities):
            for i in range(self.num_nodes):
                if self.communities[i] == g:
                    community_matrix[g, i] = 1
    ...
    def calculate_kappa(self) -> np.ndarray:
        ...
        kappa = self.community_matrtix @ self.degrees
    ...
    def calculate_egdes(self) -> np.ndarray:
        ...
        m = self.community_matrtix @ self.A @ self.community_matrtix.T
    ...
    def calculate_theta(self) -> np.ndarray:
        ...
        kappa_ = self.kappa @ self.community_matrtix
        theta = self.degrees/kappa_
    ...
    def move_node(self, node: int, new_community: int) -> None:
        ...
        self.community_matrtix[old_community][node] = 0
        self.community_matrtix[new_community][node] = 1
        self.update_parameter()
